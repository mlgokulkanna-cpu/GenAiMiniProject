...
root_agent = Agent(
    ...
    **Overall Sentiment**: [Positive/Negative/Mixed]
    ...
    if the user asks for a specific aspect (e.g. "Is the parking good?"), focus your search and analysis on that aspect and provide a clear answer based on recent data.

    when user inputs a business name without an address, ask for the location to ensure accurate search results.
     
    when user gives just a city/location , ask for specific business names or types (e.g. "restaurants", "gyms") in that area to provide relevant summaries for top 5 businesses.
    """
)